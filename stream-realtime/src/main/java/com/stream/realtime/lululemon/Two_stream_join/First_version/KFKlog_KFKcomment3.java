...
/**
 * 三流 CoProcessFunction Join 处理器 - 基于 UserID 连接
 * 3表联查
 */
public class KFKlog_KFKcomment3 {
    ...
    public static void main(String[] args) {
        ...
        env.execute("Three Stream Join with CoProcessFunction - UserID Join");
    }
    ...
    public static class CommentLogJoinResult {
        ...
        public CommentLogJoinResult(CommentData comment, LogData log) {
            ...
            if (logs != null && !logs.isEmpty()) {
                for (LogData log : logs) {
                    if (comment.userId.equals(log.userId)) {
                        System.out.println("🎯 Comment-Log JOIN 成功");
                        out.collect(new CommentLogJoinResult(comment, log));
                    }
                }
            }
        }
    ...
    /**
     * Comment-Log-UserInfo CoProcessFunction 连接处理器 - 基于 UserID
     */
    public static class CommentLogUserInfoCoProcessFunction extends CoProcessFunction<CommentLogJoinResult, UserInfoData, FinalJoinResult> {
        ...
        public void processElement1(CommentLogJoinResult commentLog, Context ctx, Collector<FinalJoinResult> out) throws Exception {
            ...
            if (userInfo != null && commentLog.userId.equals(userInfo.userId)) {
                System.out.println("🎯 最终三流 JOIN 成功");
                out.collect(new FinalJoinResult(commentLog.comment, commentLog.log, userInfo));
            }
        }
